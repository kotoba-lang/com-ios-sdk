(ns ios_sdk.main
  "Kotodama WASM entrypoint for the ios_sdk clean-room actor (L4) — Clojure port.
  ...
  facts (`ios_sdk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ios_sdk")
(def tier "L4")
...
(def entity-specs
  [{:entity "Process"   :plural "processes"   :id-prefix "iossdk_pro"
    :fields [:pid :command :state :uid]      :required [:pid :command]
    :coerce {:pid :int :uid :int}            :refs {}}
   {:entity "Thread"      :plural "threads"      :id-prefix "iossdk_thr"
    :fields [:pid :tid :state :priority]     :required [:pid :tid]
    :coerce {:pid :int :tid :int :priority :int} :refs {}}
   {:entity "FileDescriptor"     :plural "filedescriptors"     :id-prefix "iossdk_fil"
    :fields [:pid :fd :path :mode]           :required [:pid :fd]
    :coerce {:pid :int :fd :int}             :refs {}}
   {:entity "Syscall" :plural "syscalls" :id-prefix "iossdk_sys"
    :fields [:name :number :argCount]        :required [:name :number]
    :coerce {:number :int :argCount :int}    :refs {}}
   {:entity "MemoryRegion" :plural "memoryregions" :id-prefix "iossdk_mem"
    :fields [:pid :startAddr :sizeBytes :perms] :required [:pid :startAddr]
    :coerce {:pid :int :sizeBytes :int}      :refs {}}
   {:entity "Signal"    :plural "signals"    :id-prefix "iossdk_sig"
    :fields [:pid :number :action]           :required [:pid :number]
    :coerce {:pid :int :number :int}         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ios_sdk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ios_sdk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ios_sdk-compat/ready)
